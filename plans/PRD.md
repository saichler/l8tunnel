...
| C-3 | The agent authenticates with a bearer token and/or a client certificate issued by the relay's agent CA (`l8tunnel-server agent-cert issue --token T --out PREFIX`; the CA lives in the store and is created on first use). The certificate carries the token ID; its serial must still be recorded on the token, so revoking the token revokes its certificates on the next handshake. A certificate and a token presented together must name the same token. Token policy `require_cert` refuses the token string alone. (v1.1 Q1) |
...
| H-6 | **Custom domains:** an http or tls tunnel can also serve domains CNAME'd to the relay (`domains:` / `--domain`). The operator allows them per token (`token create --domains '*.example.com'`; none by default), each domain belongs to one tunnel, and domains under the base domain are refused (use the tunnel name). Routing by SNI and Host covers custom domains, with the Host/SNI rule applied per tunnel. Certificates: on demand through HTTP-01 in `acme.mode: http01`; in `dns01` the wildcard stays on DNS-01 and custom domains get HTTP-01 on demand (needs `listen.http`); with static files, a domain the certificate doesn't cover is refused at registration. Passthrough (`tls`) domains need no relay certificate. (v1.1 Q2) |
| H-7 | **Access control per tunnel**, declared in the agent's tunnel config and enforced by the relay: IP `allow_ips`/`deny_ips` (addresses or CIDRs, deny wins; every tunnel type, both modes; HTTP gets a 403 page, other types are refused) and HTTP `basic_auth` users with bcrypt hashes (`l8tunnel hash-password`; `$2a$`/`$2b$`/`$2y$` accepted). Verified credentials skip bcrypt for 5 minutes (digest cache), and the `Authorization` header is removed before the request reaches the service. **OIDC** (v1.1 Q3): providers are configured on the relay (`oidc.providers`, secrets as `${VAR}`); a tunnel picks one with `oidc: {provider, allow_emails, allow_domains}` (an allow list is required). One login host (`auth.<base>`, default) does the provider round trip with PKCE and a nonce, then hands a signed, single-use, one-minute code to the tunnel host, which sets its own HMAC-signed session cookie (Secure, HttpOnly, SameSite=Lax, 12 h default). Every request re-checks the cookie and the current allow list; the relay strips its cookie, sets `X-L8tunnel-User` to the verified email, and drops any client-sent `X-L8tunnel-User`. Non-GET requests without a session get 401 instead of a redirect. Provider discovery is lazy, so a provider outage doesn't stop the relay. |
