...
// relay-0 with one TCP or SSH tunnel to a local echo server. The realtime specs
...
export class RunningAgent {
    ...
    /** Starts an agent for token with a tunnel of this type named tunnel. */
    static async start(token: string, tunnel: string, type: 'tcp' | 'ssh' = 'tcp'): Promise<RunningAgent> {
        ...
        const proc = spawn(path.join(ENV.stateDir, 'l8tunnel-agent'), [
            ...
            type, `127.0.0.1:${port}`, '--name', tunnel
